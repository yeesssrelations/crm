...
class CrmPerson(models.Model):
    ...
    statut_personne = fields.Selection(help="Statut",
                                       selection=[('SP00', 'PROSPECT'), ('SP01', 'CLIENT'), ('SP02', 'CANDIDAT'),
                                                  ('SP03', 'FRANCHISE POTENTIEL'), ('SP04', 'FRANCHISE POTENTIEL'), ('SP05', 'MEMBER')], required=False, )
...
class Interaction(models.Model):
    ...
    interaction = fields.Many2one(comodel_name="interaction.interaction", help="Libellé Intéraction", required=False, )
    ...
    inter_us_services = fields.One2many(comodel_name="interaction.usage.service", inverse_name="pers_inter_us_service", help="Usages des services",
                                        required=False, )
...
class InteractionAbonnement(models.Model):
    ...
    code_abn = fields.Many2one(comodel_name="abonnement.abonnement", help="Code Abonnement", required=False, )
    libelle_abn = fields.Char(related="code_abn.libelle_abonnement", help="Libellé Abonnement", )
    cordonnee_abn = fields.Selection(help="Cordonnées Abonnement", selection=[('email', 'email'), ('mobile', 'mobile'), ('telephone', 'téléphone'), ],
                                     required=False, )
...
class InteractionParticipation(models.Model):
    ...
    code_operation = fields.Many2one(comodel_name="participation.participation", help="Code Opération", required=False, )
    libelle_operation = fields.Char(related="code_operation.libelle_participation", help="Libellé Opération", )
    type_operation = fields.Selection(help="Type Opération", selection=[('jeux', 'jeux'), ('concours,', 'concours'), ('activation', 'activation'), ],
                                      required=False, )
...
class InteractionCommande(models.Model):
    ...
    code_canal_cmd = fields.Char(help="Code Canal Commande", required=False, )
    libelle_canal_cmd = fields.Selection(help="Libellé Canal Commande",
    ...
    mode_payement = fields.Selection(help="Mode de paiement",
                                     selection=[('CB', 'CB'), ('Cheque', 'Chèque'), ('vir_sepa', 'virement SEPA'), ('prelev_sepa', 'prélèvement SEPA'),
                                                ('liquide', 'Liquide'), ], required=False, )
    status_cmd = fields.Selection(help="Status Commande", selection=[('payee', 'payée'), ('rejet', 'rejet'), ('anulee', 'annulée'), ('en_att', 'en attente'), ],
                                  required=False, )
...
class InteractionCoupons(models.Model):
    ...
    gencode_coupon = fields.Char(help="Code Coupon", required=False, )
...
class InteractionSollicitation(models.Model):
    ...
    nature_message_campagne = fields.Selection(help="Nature Message Campagne",
    ...
    resultat_campagne = fields.Selection(help="Résultat Campagne",
...
class InteractionUsageService(models.Model):
    ...
    code_usage_serv = fields.Many2one(comodel_name="usage.service", help="Code Usage", required=False, )
...
class InteractionContact(models.Model):
    ...
    code_int_contact = fields.Many2one(comodel_name="contact.contact", help="Code Contact", required=False, )
...
class InteractionOffre(models.Model):
    ...
    code_int_offre = fields.Many2one(comodel_name="offre.offre", help="Code Offre", required=False, )
...
class InteractionFideliteBurn(models.Model):
    ...
    code_article_burn = fields.Selection(help="Code article Burn",
                                         selection=[('Br', 'BR'), ('dotations', 'dotations'), ('cagnotage', 'cagnotage'), ('BA', 'BA'), ], required=False, )
    affection_burn = fields.Char(help="Affectation Burn", required=False, )
    solde_compte_fidelite = fields.Char(help="Solde compte fidelité", required=False, )
    statut_membre = fields.Char(help="Statut membre", required=False, )
...
class InteractionFideliteEarn(models.Model):
    ...
    affection_earn = fields.Char(help="Affectation Earn", required=False, )
    solde_compte_fidelite = fields.Char(help="Solde compte fidelité", required=False, )
    statut_membre = fields.Char(help="Statut membre", required=False, )
    pers_inter_fidel = fields.Many2one(comodel_name="person.interaction", help="Code Intéraction", required=False, )
    id_interaction_fidel = fields.Integer(related="pers_inter_fidel.id", help="Identifiant Intéraction", required=False, )
...
class Personne(models.Model):
    ...
    statut_personne = fields.Selection(help="Statut personne",
